...
def load_image(filename: str, width: int, height: int) -> pygame.Surface:
    image = pygame.image.load(str(RESOURCES_DIR / filename))
...
class Hero(pygame.sprite.Sprite):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.image = load_image("avatar.png", 45, 40).convert_alpha()
        self.velocity = [0, 0]
...
class Enemy(pygame.sprite.Sprite):
    ...
    def __init__(self, hero: Hero) -> None:
        super().__init__()
        self.hero = hero
        self.image = load_image("zombie_2.png", 45, 40).convert_alpha()
        self.speed = 50
        self.velocity = [0, 0]
        self._position = [0.0, 0.0]
        self._old_position = self.position
    ...
    def walk(self, df: float):
        hero_x = self.hero.position[0]
        hero_y = self.hero.position[1]
...
class QuestGame:
    ...
    def __init__(self, screen: pygame.Surface) -> None:
        ...
        enemies: List[Enemy] = []
        for i in range(1, 10):
            center = self.map_layer.map_rect.center
            pos_x = 0
            pos_y = 0
            if i % 2 == 0:
                pos_x = center[0] + i * 100
                pos_y = center[1] + i * 100
            else:
                pos_x = center[0] - i * 100
                pos_y = center[1] - i * 100
